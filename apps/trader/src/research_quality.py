...
class CandleQuality:
    ...
    @property
    def valid(self) -> bool:
        return not any((self.duplicates, self.invalid_ohlc, self.negative_volume, self.non_monotonic))
...
def inspect_candles(candles: list[Candle], expected_interval_ms: int) -> CandleQuality:
    ...
    invalid_ohlc = sum(1 for c in candles if c.high < max(c.open, c.close) or c.low > min(c.open, c.close))
    negative_volume = sum(1 for c in candles if c.volume < 0)
    non_monotonic = sum(1 for a, b in zip(timestamps, timestamps[1:]) if b <= a)
    gaps = sum(1 for a, b in zip(sorted(set(timestamps)), sorted(set(timestamps))[1:]) if b - a != expected_interval_ms)
    return CandleQuality(len(candles), duplicates, invalid_ohlc, negative_volume, non_monotonic, gaps)
