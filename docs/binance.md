...
`/api/account/summary` is an optional signed, read-only call to `/api/v3/account`. API keys are read from server-side environment variables, never returned to the dashboard, and no order-writing method exists in this adapter.
