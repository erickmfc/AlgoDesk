...
function App() {
  ...
  useEffect(() => {
    ...
    const loadMarket = async () => {
      try {
        ...
        setFeedStatus('live')
      } catch { setFeedStatus('offline'); setLivePrices({}) }
    }
    void loadMarket()
  ...
  return <div className="app-shell">
    ...
    <main className="main-area">
      <header className="topbar"><div className="breadcrumb"><span>ALGODESK</span><ChevronRight size={14} /><b>{navItems.find((item) => item.id === activePage)?.label.toUpperCase()}</b></div><div className="top-actions"><span className={`connection ${feedStatus === 'offline' ? 'is-offline' : ''}`}><StatusMark tone={feedStatus === 'live' ? 'cyan' : 'amber'} pulse={feedStatus === 'live'} /> {feedStatus === 'live' ? 'Binance connected' : 'Binance feed unavailable'}</span><span className="top-time">{currentTime} BRT</span><button className="icon-button" aria-label="Notificações"><Bell size={17} /></button><button className="avatar-user">AD</button><span className="user-name">AlgoTrader</span></div></header>
      ...
      <div className="content">
        ...
        <div className="stats-row"><StatCard label="Paper equity snapshot" value="$ 12,428.50" detail="Demo state · not Binance account" icon={WalletCards} /><StatCard label="Paper daily PnL" value="$ 342.18" detail="Demo state · not Binance PnL" tone="cyan" icon={CircleDollarSign} /><StatCard label="Paper drawdown" value="-3.12%" detail="Risk limit · 5.00%" tone="red" icon={Gauge} /><StatCard label="Bots online · local" value={`${botsOnline} / 5`} detail="1 paper bot halted" tone="amber" icon={Bot} /><button className={`stop-button ${hardStopped ? 'is-stopped' : ''}`} onClick={hardStopped ? () => setHardStopped(false) : stopAll}><Square size={17} fill="currentColor" />{hardStopped ? 'HARD STOP ACTIVE' : 'PARAR TODOS OS ROBÔS'}</button></div>
        ...
        {activePage === 'operations' && <div className="operations-grid">
          <section className="desk-panel"><div className="desk-panel-header"><div><span className="panel-eyebrow"><Radio size={13} /> PAPER SIMULATION</span><h2>Trading floor</h2></div><div className="desk-legend"><span><i className="legend-dot cyan" />Open</span><span><i className="legend-dot violet" />Scanning</span><span><i className="legend-dot amber" />Waiting</span><span><i className="legend-dot red" />Halted</span></div></div>
            <div className="trading-floor"><div className="floor-back-wall"><div className="wall-screen screen-map"><span>MARKET GRID</span><div className="world-dots" /></div><div className="wall-screen screen-chart"><span>BTC / ETH · BINANCE PUBLIC</span><div className="market-values"><b>₿ {livePrices.BTCUSDT ? `$${livePrices.BTCUSDT.toLocaleString('en-US', { maximumFractionDigits: 2 })}` : '—'}</b><b>Ξ {livePrices.ETHUSDT ? `$${livePrices.ETHUSDT.toLocaleString('en-US', { maximumFractionDigits: 2 })}` : '—'}</b></div><small className="market-source">{feedStatus === 'live' ? 'LIVE PUBLIC TICKER' : 'NO LIVE MARKET FEED'}</small><Sparkline /></div><div className="wall-copy">DISCIPLINE<br /><em>BEATS</em><br />EMOTION</div></div><div className="floor-grid" /><div className="floor-light light-one" /><div className="floor-light light-two" />{visibleBots.map((bot, index) => <DeskStation bot={bot} index={index} key={bot.id} onSelect={() => selectBot(bot)} selected={selectedBot?.id === bot.id} />)}<div className="floor-label">ALGODESK <span>///</span> PAPER CONTROL FLOOR</div></div>
          </section>
        ...
        <footer className="bottom-strip"><div className="allocation"><span className="strip-label">PAPER PORTFOLIO ALLOCATION</span><div className="allocation-body"><div className="donut"><span>100%</span></div><div className="allocation-keys"><span><i className="legend-dot cyan" />BTCUSDT <b>48%</b></span><span><i className="legend-dot violet" />ETHUSDT <b>32%</b></span><span><i className="legend-dot sky" />Others <b>20%</b></span></div></div></div><div className="strip-stat"><span className="strip-label">PAPER OPEN POSITIONS</span><strong>2</strong><small>Across 4 local bots</small></div><div className="strip-stat positive-stat"><span className="strip-label">PAPER REALIZED PNL · 24H</span><strong>+$ 298.65</strong><small>Snapshot · 12 trades</small></div><div className="strip-stat"><span className="strip-label">PAPER WIN RATE · 24H</span><strong>75%</strong><small>Snapshot · 9 W / 3 L</small></div><div className="strip-stat"><span className="strip-label">AVG. TRADE DURATION</span><strong>1h 24m</strong><small>Paper execution</small></div><div className="system-status"><span className="strip-label">SYSTEM STATUS</span><div><StatusMark tone={feedStatus === 'live' ? 'cyan' : 'amber'} pulse={feedStatus === 'live'} /> {feedStatus === 'live' ? 'Binance public feed' : 'Binance feed unavailable'}</div><div><StatusMark tone={feedStatus === 'live' ? 'cyan' : 'amber'} pulse={feedStatus === 'live'} /> {feedStatus === 'live' ? 'Market data live' : 'Market data offline'}</div><div><StatusMark tone={hardStopped ? 'red' : 'amber'} /> {hardStopped ? 'Paper engine halted' : 'Paper engine running'}</div></div><div className="strip-pulse"><Sparkline /><span>{feedStatus === 'live' ? 'STABLE' : 'OFFLINE'}</span></div></footer>
      </div>
    </main>
